...
(function () {
  ...
  ProctorSession.prototype._onMsg = function (m) {
    switch (m.type) {
      ...
      // Phase 6: Session control events from Decision Engine
      case 'session:pause':
        console.log('⏸ Session PAUSED by agent:', m.data && m.data.reason);
        this._showOverlay('paused', m.data && m.data.reason || 'Session paused by integrity system', m.data && m.data.canResume);
        this.emit('session:paused', m.data);
        break;
      ...
      case 'session:resume':
        ...
        this._removeOverlay();
        this._setStatus('Monitoring Active', '#10b981');
        this.emit('session:resumed', m.data);
        break;

      case 'session:terminate':
        console.log('🛑 Session TERMINATED by agent:', m.data && m.data.reason);
        this.isActive = false;
        this._showOverlay('terminated', m.data && m.data.reason || 'Session terminated by integrity system', false);
        this._setStatus('Terminated', '#ef4444');
        this.emit('session:terminated', m.data);
        // Auto-submit partial answers if form exists
        this._autoSubmitPartial();
        break;

      case 'session:lock':
        console.log('🔒 Session LOCKED by agent:', m.data && m.data.reason);
        this._showOverlay('locked', m.data && m.data.reason || 'Session locked — admin intervention required', false);
        this.emit('session:locked', m.data);
        break;
    }
  };
  ...
  /* ================================================================
   *  Phase 6: Session Control Overlays
   * ================================================================ */
  ProctorSession.prototype._showOverlay = function (type, reason, canResume) {
    this._removeOverlay();
    ...
    var bgColor = type === 'terminated' ? '#dc2626' : type === 'locked' ? '#ea580c' : '#f59e0b';
    var icon = type === 'terminated' ? '🛑' : type === 'locked' ? '🔒' : '⏸';
    var title = type === 'terminated' ? 'Session Terminated' : type === 'locked' ? 'Session Locked' : 'Session Paused';
    var subtitle = type === 'terminated' ? 'This exam session has been ended by the integrity system.'
      : type === 'locked' ? 'This session requires administrator intervention.'
      : 'Your exam has been temporarily paused.';

    overlay.innerHTML =
      '<div style="text-align:center;max-width:480px;padding:40px;">' +
        '<div style="font-size:64px;margin-bottom:16px;">' + icon + '</div>' +
        '<h2 style="color:' + bgColor + ';font-size:28px;font-weight:700;margin-bottom:12px;font-family:inherit;">' + title + '</h2>' +
        '<p style="color:#d1d5db;font-size:16px;margin-bottom:16px;line-height:1.5;font-family:inherit;">' + subtitle + '</p>' +
        '<div style="background:rgba(255,255,255,0.1);border-radius:8px;padding:16px;margin-bottom:20px;">' +
          '<p style="color:#9ca3af;font-size:13px;font-family:inherit;">Reason: ' + (reason || 'No reason provided') + '</p>' +
        '</div>' +
        (canResume
          ? '<p style="color:#6b7280;font-size:13px;font-family:inherit;">Waiting for administrator to resume your session...</p>'
          : type === 'terminated'
            ? '<p style="color:#6b7280;font-size:13px;font-family:inherit;">Your partial answers have been submitted. Contact your administrator for next steps.</p>'
            : '<p style="color:#6b7280;font-size:13px;font-family:inherit;">Please wait for administrator action.</p>') +
      '</div>';
    ...
    overlay.addEventListener('keydown', function (e) { e.preventDefault(); e.stopPropagation(); }, true);
  };

  ProctorSession.prototype._removeOverlay = function () {
    var existing = document.getElementById('proctor-overlay');
    if (existing) existing.remove();
  };

  ProctorSession.prototype._autoSubmitPartial = function () {
    // Try to find and click a submit button on the quiz form
    var submitBtn = document.querySelector('[data-proctor-submit]') ||
                    document.querySelector('button[type="submit"]');
    if (submitBtn) {
      console.log('📝 Auto-submitting partial answers due to termination');
      submitBtn.click();
    }
    // Clean up proctoring after short delay
    var self = this;
